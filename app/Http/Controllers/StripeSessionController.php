...
{
    public function createCheckoutSession(Request $request,$product_id,$shop_id,$quantity)
    {
        $shop_name = str_replace(' ', '', $shop_id.'.myshopify.com');
        $shop_token = User::where('name', $shop_name)->first();
        $shop_token = $shop_token->password;
        $curl = curl_init();  
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'https://'.$shop_name.'/admin/api/2023-01/products/'.$product_id.'.json',
        ...
        $session = Session::create([
            ...
            'mode' => 'payment',
            'success_url' =>  'https://2b4d-61-247-229-81.in.ngrok.io/success/?title='.$product_title.'&quantity='.$quantity.'&product_price='.$product_price.'&final_price='.$final_price.'&total_grams='.$total_grams.'&session_id='.'{CHECKOUT_SESSION_ID}/&shop='.$shop_id,
    ...
    public function checkoutSuccess(Request $request,$title,$quantity,$product_price,$final_price,$total_grams,$session,$shop)
    {
    $shop_name = str_replace(' ', '', $shop_id.'.myshopify.com');
    $shop_token = User::where('name', $shop_name)->first();
    $shop_token = $shop_token->password;
